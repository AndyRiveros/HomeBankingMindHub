    ...
    public class LoansController : ControllerBase
    {
        ...
        public IActionResult CreateLoanToCurrent([FromBody] LoanApplicationDTO loanApplication)
        {
            try
            {
                ...
                if (loan == null) return StatusCode(403, "Cant find Loan");

               
                
                //check amount
                if (loanApplication.Amount < 1 || loanApplication.Amount > loan.MaxAmount)
                    return StatusCode(403, $"El monto debe ser entre 1 y {loan.MaxAmount}");
               
                
                //check payments
                List<int> paymentOptions = loan.Payments.Split(',').Select(int.Parse).ToList();
                if (!paymentOptions.Contains(loanApplication.Payments))
                    return StatusCode(403, $"available installments for loan {loan.Name} are {loan.Payments}");
                ...
                Transaction newLoanDeposit = new Transaction
                {
                    ...
                    Description = $"{loan.Name} loan approved succesfully",
                ...
                return Created("loan created with success", newClientLoan);
            }
            catch (Exception ex)
            {
                return StatusCode(500, ex.Message);
